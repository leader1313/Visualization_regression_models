...
class IOMGP:
    def __init__(self, X, Y, kernel, Max_mixture=2, K=torch.tensor([]), lengthscale=1.0):
        """
        Argument
                X : State (N X Q)
                Y : Action (N X D)
        ...
        # )  # K X D
        self.log_sigma = torch.stack(
            [torch.tensor(np.log([0.8] * self.D)).float()
    ...
    def variationalBound(self, n_batch=None):
        ...
        term1 = (
            torch.log(np.pi * 2 * Psi).permute(2, 0,
                                               1).view(self.D, self.M * self.N)
        )
