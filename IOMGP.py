...
class IOMGP:
    def __init__(self, X, Y, kernel, Max_mixture=2, K=[]):
        '''
        Argument
                X : State (N X Q)
                Y : Action (N X D)
        ...
        lengthscales = torch.log(
            (self.X.max() - self.X.min()) * 1.0).unsqueeze(0)
        ...
        self.log_sigma = torch.stack(
            [
                torch.tensor(np.log([1.0] * self.D)).float() for _ in range(len(self.K))
            ])  # K X D
    ...
    def update_Psi(self):
        # N X D
        Psi = torch.cat([
            self.log_sigma[k].exp().repeat(self.K[k], 1)
    ...
    def precompute(self, n_batch=None):
        ...
        R = sqrtB.matmul(K).matmul(sqrtB) +\
            torch.eye(ind.shape[0]).repeat(self.D, self.M, 1, 1)
    ...
    def update_q_z(self):
        ...
        term2 = -0.5 * (
            ...
            + torch.diagonal(self.q_f_sig, dim1=2, dim2=3).permute(
                1, 2, 0).mul(1 / Psi).sum(2))
    ...
    def update_q_f(self):
        ...
        self.q_f_mean = self.q_f_sig.matmul(
            B).matmul(self.Y.repeat(self.M, 1, 1).permute(2, 0, 1).unsqueeze(3)).\
            squeeze(3).permute(1, 2, 0)
    ...
    def variationalBound(self, n_batch=None):
        ...
        lk_1 = torch.slogdet(R)[1].sum() +\
            0.5 * Y.matmul(self.sBRsB).matmul(Y.transpose(2, 3)).sum()

        # likelihood term 2-------------------

        term1 = torch.log(np.pi * 2 * Psi).permute(2, 0, 1).\
            view(self.D, self.M * self.N)
        ...
        E_ln_1_minus_v = torch.digamma(
            v_beta_b) - torch.digamma(v_beta_a + v_beta_b)
        ...
        for m in range(0, self.M):
            ...
            E_zv -= (self.q_z_pi[m][ind]
                     [None, ...].mm(r[m][ind][..., None])).sum()
    ...
    def predict(self, x):
        ...
        mean = Knx.transpose(1, 2).matmul(self.sBRsB).matmul(
            self.Y.repeat(self.M, 1, 1).permute(2, 0, 1).unsqueeze(3))\
            .squeeze(3).permute(1, 2, 0)

        sigma = (Kx - Knx.transpose(1, 2).matmul(self.sBRsB)
                 .matmul(Knx)).diagonal(dim1=2, dim2=3).permute(1, 2, 0) + \
            self.Psi[-1, :].repeat(self.M, x.shape[0], 1)
...
if __name__ == "__main__":
    ...
    num_graph = GP_ok.sum()
    xx = torch.linspace(min(model.X)[0], max(
        model.X)[0], steps=100).unsqueeze(1)
    ...
    for m in range(num_graph):
        ...
        #         hatch="\\",
        #     )
    plt.scatter(model.X, model.Y, c="tomato",
                marker="*", s=200, label="Training Data")
